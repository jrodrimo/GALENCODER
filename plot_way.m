...
% load('dron_position.mat') %fit format to GO++ format
% %  dron_position.latitud./1e5
% %  dron_position.longitud./1e5
%  
% dron_coor(1,:)=linspace(bounds(1,1),bounds(1,2),50); %latitude data
% dron_coor(2,:)=linspace(bounds(2,1),bounds(2,2),50);   %longitude data
% plot(hax, dron_coor(1,:), dron_coor(2,:), '.m')
% 
% plot(hax,  dron_position.latitud, dron_position.longitud, '*k')
    %waitforbuttonpress
end
disp(key_catalog.')
